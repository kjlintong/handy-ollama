...
1. **（本轮）** C1/C2/C6 风格统一:C6/1、C6/2「简介/结论」→导语+「本节小结+延伸阅读」,二级标题 1.-4. → 中文序号;补本节定位;C6/2 notebook 引用补链接;C2/1-4 四节补版本基线+本节定位+本节小结+延伸阅读(「参考链接」→「延伸阅读」),「简介」并入导语,修「运行Ollama/退出Ollama/docker 篇/笔者」等口吻与空格;C1/1 保留「引言+0.x」引子设计(变通);全库 0 坏链接。
2. **5514447** 第九章第 2~4 节重构为新风格:段落标题统一为中文序号(「一、环境设置」…);C9/2 删除英文残留(「You can also see this page…」完整嵌入模型列表句 + 三行 Note 噪音)并拟人化机翻句;三节结尾「总结/参考文献」→「本节小结+延伸阅读」;版本基线补「本节定位」;「>注:」格式统一;C9/4 旧 GitHub docs/C7 链接与 Step 序号改本地引用/中文步骤;同章引用 ../C9/ → ./;v0.2 嵌入链接升级新版(概念类 v0.2 链接保留,与教程 API 配套)。
3. **e3851d8** 第七章第二节:措辞拟人化(「每一步都是上一步的超集」→「后面的版本只加功能、不删功能」),删第五节标题「（诚实清单）」。
4. **2f205c7** 章节大重组:第七章(Agent 架构原理)并入原第八章 → 新**第7章 Agent 原理与轻量级实现**;原 9→8、10→9、11→10、12→11;物理目录 C8-C12 → C7-C11;删除各节冗余「本节范围」块;migration-map 的 C3 过时映射一并修正。
5. **1ae00ff** 第五章+全库:C5 病句修复(「薄胶水模式」首次出现即解释,5.3 展开为两步);「章节定位/本章定位」统一「本节定位」;清理版本基线/定位/边界自检中的作者口吻(写作时/改版刷新/边界铁律/核心新增等);「边界自检」节更名「本节范围」。
6. **05a0aaa** 第四章:「本章小结」全库统一「本节小结」(25 处);修 4.1 目录锚点(#四删除模型→#四复制模型)、给 4.1 补本节小结+延伸阅读;4.2 引用编号笔误(C2.5→C2.2);4.4 与附录 A-0 的 JS notebook 错误路径(notebook/C5→—)。
7. **bfa228b** 第三章重组:C2/5 GPU-CPU 混合推理配置移入 C3/5;删除过时的「自定义在 GPU 中运行」(错误默认 CPU + 废弃 OLLAMA_GPU_LAYER);「自定义导入模型」重写去重(删 GGUF 原理重复、删自定义 Prompt 整节、use_auth_token→token、修 HF_ACCESS_TOKE 笔误);「存储位置」整理编号错乱;全库 10 处引用同步,顺带修 C1 链接多空格的 %20 与 README ../docs 越界链接。
8. **296cd54**(云端提交,已同步本地)删除 docs/C1/1. Ollama 介绍.md。
...
- [ ] 第 10、11 章(新编号)内容尚未逐节细审(用户按章推进,已检 C3-C9、C1/C2/C6 风格已统一)。
