...
以 Windows 系统为例，Ollama 拉取的模型默认存储在 C 盘，若拉取多个模型容易让 C 盘爆满。本节介绍如何在 Windows、Linux 和 macOS 三大主流操作系统中自定义 Ollama 模型的存储位置。

核心思路只有一个：**通过 `OLLAMA_MODELS` 环境变量指定模型目录**。按场景分为三种做法：
...
> 默认情况下，模型会下载到 `C:\Users\<用户名>\.ollama\models`
>
> 如果不想放在 C 盘，先创建自定义目录并复制路径，如 `D:\Ollama\Models`
>
> ❗注意：路径需提前创建，且不含中文或空格
...
> 适用场景：安装 Ollama 完成后，希望修改模型的存储位置，或希望新下载的模型直接存放到新位置。
...
如果 Ollama 已经作为 systemd 服务运行，可以通过以下命令重启：

```bash
sudo systemctl restart ollama
```

如果是以 `ollama serve` 手动启动的，重启该进程即可（Ctrl+C 停止后重新运行 `ollama serve`）。
