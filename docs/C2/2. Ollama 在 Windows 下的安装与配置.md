...
> **版本基线**：Ollama 0.17.x（Windows 11 实测,本书实测环境为「Windows 11 + Ollama 0.17.7（Windows 侧服务）+ WSL2 访问」）
> **本节定位**：Windows 上安装 Ollama、配置环境变量,并解决 WSL2 访问 Windows 侧服务的常见问题。

本节完成 Windows 上 Ollama 的安装与配置,分为：官网下载、环境变量配置、运行、验证安装。
...
| OLLAMA_NUM_GPU | 表示加载到 GPU 的层数，默认 -1（全部层加载到 GPU）<br />设置部分层数可让 GPU+CPU 混合推理，见 [第3章 3.5](../C3/5.%20GPU-CPU%20混合推理配置.md) |
...
## 五、如何在 WSL2（Windows 子系统 Linux）中访问

很多开发者习惯在 WSL2 里跑代码，但 Ollama 装在 Windows 侧。此时 **WSL 里不能直接用 `localhost:11434`**（WSL2 是独立的虚拟网络），需要走 Windows 主机的网关 IP：

```bash
# 在 WSL 中查询 Windows 侧网关 IP（每次启动 WSL 可能变化）
ip route show default | awk '{print $3}'
# 输出示例: 172.28.176.1

# 用网关 IP 访问 Ollama（需 Windows 侧设置 OLLAMA_HOST=0.0.0.0:11434）
curl http://172.28.176.1:11434/api/tags
```

把网关 IP 导出为环境变量，日常使用更方便：

```bash
export OLLAMA_HOST_IP=$(ip route show default | awk '{print $3}')
curl http://$OLLAMA_HOST_IP:11434/api/tags
```

> 也可以反过来：在 WSL2 内直接安装 Linux 版 Ollama，两者互不冲突，按需选用。
> 本书的实测环境即为「Windows 11 + Ollama 0.17.7（Windows 侧服务）+ WSL2 访问」。

## 六、如何取消开机自启

（2026 年刷新）目前 Ollama 在 Windows 系统下默认开机自启，如果不需要开机自启，可以通过以下步骤取消。
...
- 安装：官网下载 `OllamaSetup.exe`，双击安装即默认开机运行；
- 关键配置：`OLLAMA_MODELS` 建议移到非 C 盘；需要局域网访问时设 `OLLAMA_HOST=0.0.0.0`；
- WSL2 访问：走 Windows 主机网关 IP（`ip route show default`），Windows 侧需 `OLLAMA_HOST=0.0.0.0:11434`；
- 取消开机自启：任务管理器 → 启动应用 → 禁用 `ollama.exe`。
