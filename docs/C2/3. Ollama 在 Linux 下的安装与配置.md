...
> **版本基线**：Ollama 0.17.x（Linux 服务安装,以 systemd 托管）
> **本节定位**：Linux 上安装 Ollama 的两种方式,以及更新、固定版本、看日志、卸载等日常运维操作。

本节完成 Linux 上 Ollama 的安装与配置：快速安装、手动安装,以及更新、特定版本安装、查看日志和卸载。
...
- 快速安装：`curl -fsSL https://ollama.com/install.sh | sh` 一行完成；
- 手动安装：下载官方二进制 + 注册 systemd 服务（支持 GPU 检测与开机自启）；
- 运维四件事：`ollama update` 更新、固定版本安装、`journalctl -u ollama` 看日志、卸载四步清除干净。

## 延伸阅读

- 其他平台安装：[2.1 macOS](./1.%20Ollama%20在%20macOS%20下的安装与配置.md) / [2.2 Windows](./2.%20Ollama%20在%20Windows%20下的安装与配置.md) / [2.4 Docker](./4.%20Ollama%20在%20Docker%20下的安装与配置.md)
