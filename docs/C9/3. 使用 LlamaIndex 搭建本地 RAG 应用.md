...
> **版本基线**：Ollama 0.17.x + llama-index 0.10.x（与 [9.1 RAG 原理](./1.%20RAG%20原理与本地组件.md) 配套的实战章节）
> **本节定位**：LlamaIndex 路线实战——加载本地文档、构建索引、检索上下文对话，模型与嵌入全靠 Ollama。

本文用 LlamaIndex 搭建本地 RAG：加载本地文档 → 构建向量索引 → 基于检索结果对话。可以自定义本地知识库路径，通过 LlamaIndex 构建索引，然后利用索引进行上下文对话。
...
- 三步上手：加载数据 → `Settings` 全局指定 Ollama 的嵌入/LLM → `VectorStoreIndex` 构建索引；
- 对话两种方式：`as_query_engine()` 单轮问答，`as_chat_engine()` 带记忆多轮对话（用 `ChatMemoryBuffer` 控制 token 占用）；
- 索引可持久化：`persist` 存储、`load_index_from_storage` 加载，避免每次重新构建。

## 延伸阅读

- [LlamaIndex 官方文档](https://docs.llamaindex.ai/en/stable/getting_started/starter_example_local/?h=ollama#load-data-and-build-an-index)