...
> **版本基线**：Ollama 0.17.x（与 [9.1 RAG 原理](./1.%20RAG%20原理与本地组件.md) 配套的实战章节）
> **本节定位**：LangChain 路线实战——用 Ollama 完成「文档分块 → 向量化 → 检索 → 生成」的完整本地 RAG。

开始之前，建议先熟悉以下 LangChain 概念：
...
本地运行大语言模型的重要性日益凸显，[llama.cpp](https://github.com/ggerganov/llama.cpp)、[Ollama](https://github.com/ollama/ollama)、[llamafile](https://github.com/Mozilla-Ocho/llamafile) 等流行项目都是例证。
...
> 包安装完成后若提示重启内核（Note: you may need to restart the kernel to use updated packages），重启 Jupyter kernel 即可。可用的嵌入模型完整列表见 [LangChain Embedding 模型集成文档](https://python.langchain.com/docs/integrations/text_embedding/)。

## 二、文档加载

现在让我们加载并分割一个示例文档。

我们将以 Lilian Weng 关于 Agent 的 [博客](https://lilianweng.github.io/posts/2023-06-23-agent/) 为例。
...
- 完整链路：文档加载 → 分块 → 向量化（`nomic-embed-text`）→ 检索 → 本地生成（`llama3.1:8b`），全程不依赖云端 API；
- 两种 QA 形态：「简单 QA」直接拼接检索结果，「带检索的 QA」由 retriever 自动检索——后者才是 RAG 的标准形态；
- 想要换模型、调效果，只需改 `model` 与嵌入模型名称，再调整分块参数即可。

## 延伸阅读

- RAG 原理与组件选型：[9.1 RAG 原理与本地组件](./1.%20RAG%20原理与本地组件.md)
- LlamaIndex 路线：[9.3 使用 LlamaIndex 搭建本地 RAG 应用](./3.%20使用%20LlamaIndex%20搭建本地%20RAG%20应用.md)
- DeepSeek R1 路线：[9.4 使用 DeepSeek R1 和 Ollama 实现本地 RAG 应用](./4.%20使用%20DeepSeek%20R1%20和%20Ollama%20实现本地%20RAG%20应用.md)
