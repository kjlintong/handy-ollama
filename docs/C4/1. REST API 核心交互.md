# Ollama REST API 核心交互

> **版本基线**：Ollama 0.17.x
> **本节定位**：聚焦 REST API 的请求/响应结构，掌握与本地模型交互的核心模式。OpenAI 兼容接口与 Tool Calling 分别见 [2. OpenAI 兼容接口](./2.%20OpenAI%20兼容接口.md) 与 [3. Tool Calling API](./3.%20Tool%20Calling%20API.md)。
...
```bash
curl http://localhost:11434/api/generate -d "{\"model\": \"qwen3:8b\", \"prompt\": \"为什么草是绿的\"}"
```
...
{
  "model":"qwen3:8b",
  "created_at":"2024-08-08T02:54:9.819603411Z",
  "response":"",
  "done":true,
  "done_reason":"stop",
  "context":[1,2,3],
...
### 高级玩法
[非流式输出](#非流式输出)、[JSON模式](#json模式)、[多模态输入](#输入包含图片)、[可复现输出](#可复现输出)的参数设置与「回答补全」的一致。
#### 带历史记录
...
- **两大对话端点**：`/api/generate`（文本补全）与 `/api/chat`（多轮对话，支持 `tools` 工具调用）；
- **模型管理**：`create` / `copy` / `delete` / `tags` / `show` / `pull` / `push` 覆盖模型全生命周期；
- **运行时查询**：`/api/ps` 查看驻留模型，`/api/embed` 生成嵌入（RAG 场景）；
- **通用模式**：流式/非流式（`stream`）、JSON 输出（`format`）、参数透传（`options`）、驻留控制（`keep_alive`）；
- 流式响应取最后一个 `done: true` 的对象获取完整统计（token 数与耗时）。

## 延伸阅读

- 官方 API 参考：[https://docs.ollama.com/api](https://docs.ollama.com/api)
- OpenAI 兼容接口：[2. OpenAI 兼容接口](./2.%20OpenAI%20兼容接口.md)
- 工具调用：[3. Tool Calling API](./3.%20Tool%20Calling%20API.md)
- 各语言 SDK 调用：[4. 多语言调用（附录）](./4.%20多语言调用（附录）.md)
