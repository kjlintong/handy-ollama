...
> **版本基线**：Ollama 0.17.x（示例基于 qwen3:8b）
> **本节定位**：LangChain 的 JavaScript 集成——ChatOllama 对话、多模态、工具调用,以及高级的 JSON 输出与知识图谱生成。

本文介绍如何在 JavaScript 环境中用 LangChain 集成 Ollama，创建本地 AI 应用：Ollama 负责模型部署，LangChain（JS 版）提供应用框架。

> 注: 本文档包含核心代码片段和详细解释。完整代码可在 [Jupyter notebook](https://github.com/datawhalechina/handy-ollama/blob/main/notebook/C5/ollama_langchain_javascript) 中找到。
...
3. 通过 `ollama pull <name-of-model>` 命令获取可用 LLM 模型（例如：`ollama pull qwen3:8b`）。
...
- 能力覆盖：ChatOllama 对话、多模态、工具调用,以及高级的 JSON 输出与知识图谱生成；
- 与 Python 版能力对齐：同一套 LangChain 抽象在 JS 生态同样可用，模型仍由本地 Ollama 提供（`qwen3:8b`）；
- 灵活组合：按场景选择模型与组件，无需云端 API 即可构建从对话到知识图谱的各类应用。

## 延伸阅读

- Python 版：[6.1 LangChain 集成 - Python](./1.%20LangChain%20集成%20-%20Python.md)
- LlamaIndex 路线：[6.3 LlamaIndex 集成](./3.%20LlamaIndex%20集成.md)
- 对接任意框架：[6.4 OpenAI 兼容接口对接任意框架](./4.%20OpenAI%20兼容接口对接任意框架.md)
