...
> **本节定位**：LangChain 的 Python 集成——对话、流式输出、工具调用、多模态,以及进阶的 RAG 问答,全部基于本地 Ollama。

本文介绍如何在 Python 环境中用 LangChain 集成 Ollama，创建本地 AI 应用：Ollama 负责模型部署，LangChain 提供应用框架，两者结合即可在本地快速搭建从对话到 RAG 的各类应用。
...
先初始化模型，ChatPromptTemplate 允许我们创建一个可重用的模板，其中包含一个或多个参数。这些参数可以在运行时动态替换，以生成不同的提示。
...
- 能力覆盖：对话（ChatPromptTemplate）、流式输出、工具调用、多模态；进阶用法含 ConversationChain 与基于 FAISS 的本地 RAG；
- 灵活组合：按场景选择模型与组件，`qwen3:8b` 即可支撑全部示例，无需云端 API；
- RAG 链路：文本分割 → OllamaEmbeddings 向量化 → FAISS 索引 → 检索 → 生成回答。

## 延伸阅读

- JavaScript 版：[6.2 LangChain 集成 - JavaScript](./2.%20LangChain%20集成%20-%20JavaScript.md)
- LlamaIndex 路线：[6.3 LlamaIndex 集成](./3.%20LlamaIndex%20集成.md)
- RAG 完整实战：第 9 章「本地 RAG 与知识库」
