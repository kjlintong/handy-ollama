...
> **版本基线**：Ollama 0.17.x + Dify
> **本节定位**：Dify 可视化接入本地模型——配置 LLM 与 Embedding 两条链路,并解决 Docker 部署时连不上 Ollama 的经典问题。
...
2. 运行 Ollama 并与 Llama 聊天
    ...
    其他模型可访问 [Ollama Models](https://ollama.com/library) 了解详情。
...
5. 使用 Ollama 模型
   ...
   进入需要配置的 App 提示词编排页面，选择 Ollama 供应商下的 `qwen3:8b` 模型，配置模型参数后即可使用。
...
- 接入路径：`设置 > 模型供应商 > Ollama`，填模型名与基础 URL（LLM 与 Embedding 各配一次）；
- Docker 部署 Dify 时不能用 `localhost`（容器内指向自身）——填局域网 IP（`192.168.x.x`）或 `host.docker.internal`；
- 开放 Ollama 服务：macOS 用 `launchctl setenv`、Linux 用 systemd `Environment=`、Windows 用系统环境变量，统一设 `OLLAMA_HOST=0.0.0.0`。
